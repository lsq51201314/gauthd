...
public final class MatrixPasswd extends Rpc {
  ...
  public void Server(Rpc.Data paramData1, Rpc.Data paramData2, Manager paramManager, Session paramSession) throws ProtocolException {
    MatrixPasswdArg matrixPasswdArg = (MatrixPasswdArg)paramData1;
    MatrixPasswdRes matrixPasswdRes = (MatrixPasswdRes)paramData2;
    try {
      System.out.println("GQueryPasswd:account is " + matrixPasswdArg.account.getString() + " , login ip is " + matrixPasswdArg.loginip);
      Object[] arrayOfObject1 = storage.acquireIdPasswd(matrixPasswdArg.account.getString());
      if (arrayOfObject1 == null) {
        GAuthServer.GetLog().info("GQueryPasswd:can not find user " + matrixPasswdArg.account.getString());
        matrixPasswdRes.retcode = 2;
        return;
      } 
      ...
      matrixPasswdRes.response.replace((byte[])arrayOfObject1[1]);
      matrixPasswdRes.userid = ((Integer)arrayOfObject1[0]).intValue();
      matrixPasswdRes.retcode = 0;
      matrixPasswdRes.algorithm = 0;
      Integer integer = new Integer(matrixPasswdRes.userid);
      ...
      if (!bool) {
        int i = storage.checkIPLimit(integer, new Integer(bswap(matrixPasswdArg.loginip)));
        if (i == 1 || i == 2) {
          initReason();
          GRoleForbid gRoleForbid = new GRoleForbid();
...
/* Location:              D:\UserData\Desktop\authd\!\protocol\MatrixPasswd.class
